...
# Lee automáticamente el secreto configurado bajo el formato [connections] en Streamlit Cloud
try:
    conn = st.connection("gsheets", type=GSheetsConnection)
    # cachea los datos por 1 minuto para actualizar rápido los cambios de tu planilla
            ...
            for idx, row in socios_filtrados.iterrows():
                with st.container():
                    ...
                    if st.session_state.get(f"ver_pago_{idx}", False):
                        st.warning("### 💳 Instrucciones Manuales para Desbloquear Contacto")
                        st.write(
